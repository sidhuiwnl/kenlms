...
import face from "../../../../assets/Sibu.jpg";
...
export function Indiviualdashboardmain() {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_REACT_APP_API_URL}user/user/${id}`)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${import.meta.env.VITE_REACT_APP_API_URL}user/userworkhour/${id}?weekStart=${weekDays[0].date}&weekEnd=${weekDays[6].date}`
      )
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_REACT_APP_API_URL}user/assessment-logs/${id}`)
      .then((res) => {
